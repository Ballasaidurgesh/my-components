...
function MultiSelect({
  ...
  const renderValue = (value: unknown) => {
    ...
    // Show placeholder when no items are selected or only placeholder is selected
    ...
    // Show 'Selected All' when all items are selected
    if (
      selected.includes("select-all") ||
      (selected.length === options.length && !disableSelectAll)
    ) {
      return "Selected All";
    }
  ...
  return (
    <div className="input-container">
      ...
      <CustomSelect
        ...
        value={inputValue.length > 0 ? inputValue : ["placeholder"]}
        disabled={disabled}
